...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      ADC初始化
//  @param      ADC_IN0_Ax              选择ADC通道，见wh_adc.h内关于ADC通道的定义
//  @return     void
...
void adc_init(ADCCH_enum ADC_INx_xx)
{
    ...
    /* 开启GPIO、ADC1的时钟 */
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA |RCC_APB2Periph_ADC1 , ENABLE );   //使能ADC1通道时钟，默认采用ADC1
...
uint16 adc_convert(ADCCH_enum ADC_INx_xx, ADCRES_enum resolution)
{
    ...
    ADC_RegularChannelConfig(ADC1, (uint8)ADC_INx_xx, 1, ADC_SampleTime_1Cycles5);  //ADC1,ADC通道,采样时间为1.5周期
...
//-------------------------------------------------------------------------------------------------------------------
//  @brief      ADC引脚初始化
//  @param      ADC_IN0_Ax          可选择范围由wh_adc.h内ADCCH_enum枚举值确定
//  @return     void
